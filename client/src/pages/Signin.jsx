...
import { artistSigninFailure, artistSigninStart, artistSigninSuccess } from '../redux/users/artistSlice.js';
...
const Signin = () => {
  ...
  // ✅ Manage new UI states
  const [rememberMe, setRememberMe] = useState(false);
  const [showPassword, setShowPassword] = useState(false);

  const { error, loading } = useSelector((state) => state.artist);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      if (!data.success) {
        dispatch(artistSigninFailure(data.message || "Signin failed"));
        return;
      }

      if (data.user.role === "artist") {
        // ✅ Save artist token separately
        saveToken("artistUser", data.token);

        // ✅ Update Redux slice
        dispatch(artistSigninSuccess({ ...data.user, token: data.token }));
        
        navigate('/home'); // redirect after signin
      } else {
        dispatch(artistSigninFailure("Access denied"));
      }
    } catch (err) {
      dispatch(artistSigninFailure(err.message));
    }
  };
  ...
  return (
    ...
    <div className="h-[100dvh] w-full bg-black overflow-y-auto text-white relative">
      ...
      {/* 🌌 VOLUMETRIC BACKGROUND LIGHTING (Moved inside to scroll with content) */}
      ...
      {/* 🔥 THE FIX: The inner div ensures content is centered, but stretches if it needs to scroll */}
      <div className="min-h-full flex flex-col items-center justify-center p-6 py-12 relative z-10">
        
        {/* 🛡️ THE ARENA AUTH PORTAL */}
        <motion.div 
          initial={{ opacity: 0, y: 40, scale: 0.98 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }}
          // Removed 'my-auto' because the parent container now handles centering perfectly
        ...
        >
          ...
          <div className="text-center mb-8 relative z-30">
            <ShieldCheck className="text-yellow-500/20 mx-auto mb-3" size={32} strokeWidth={1} />
            <h1 className="text-4xl font-black italic uppercase tracking-tight leading-none">
              Yenuvia <span className="text-yellow-500">SigIn Portal</span>
            </h1>
            <p className="text-zinc-600 text-[9px] font-black uppercase tracking-[0.3em] mt-2 leading-tight">Authenticating your Identity</p>
          </div>
          ...
          <form onSubmit={handleFormSubmit} className="flex flex-col gap-6 relative z-30">
            ...
            <div className="flex items-center gap-3 ml-2 -mt-1">
              <div 
                ...
                className={`w-5 h-5 rounded-md border-2 cursor-pointer transition-colors flex items-center justify-center ${rememberMe ? 'bg-yellow-500 border-yellow-400' : 'border-white/10 bg-zinc-900'}`}
              >
                {rememberMe && <ArrowRight size={14} className="text-black" strokeWidth={3} />}
              </div>
              <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest cursor-pointer" onClick={() => setRememberMe(!rememberMe)}>Remember Identity</span>
            </div>
            ...
            <div className="flex items-center gap-4 my-1 opacity-50">
              ...
              <span className="text-[9px] font-black uppercase tracking-widest text-zinc-700">Or validate with</span>
              <div className="flex-1 h-px bg-white/10" />
            </div>
